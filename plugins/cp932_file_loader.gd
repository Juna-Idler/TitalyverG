...
func _open(_title : String, _artists : PackedStringArray, _album : String,
		file_path : String,_meta : Dictionary) -> Control:
	...
	if FileAccess.file_exists(kra_path):
		var bytes := FileAccess.get_file_as_bytes(kra_path)
		var sjis := translate(bytes)
		if not sjis.is_empty():
			result.append(sjis)
	var lrc_path = base_name + ".lrc"
	if FileAccess.file_exists(lrc_path):
		var bytes := FileAccess.get_file_as_bytes(lrc_path)
		var sjis := translate(bytes)
		if not sjis.is_empty():
			result.append(sjis)
	var txt_path = base_name + ".txt"
	if FileAccess.file_exists(txt_path):
		var bytes := FileAccess.get_file_as_bytes(txt_path)
		var sjis := translate(bytes)
		if not sjis.is_empty():
			result.append(sjis)
	loaded.emit(result,"failed" if result.is_empty() else "success")
	return null


func translate(bytes : PackedByteArray) -> String:
	var result : String = ""
	var i := 0
	while i < bytes.size():
		var first := first_table[bytes[i]]
		if first > 0:
			result += char(first)
			i += 1
		else:
			result += char(table[(bytes[i] << 8) + bytes[i+1]])
			i += 2
	return result


func make_table():
	first_table.resize(256)
	var line := TABLE.split("\n")
	for l in line:
		if not l.begins_with("0x"):
			continue
		var column := l.split("\t")
		var code := column[0].hex_to_int()
		if code <= 0xFF:
			if column[1].begins_with("0x"):
				first_table[code] = column[1].hex_to_int()
			else:
				first_table[code] = -1 if column[2] == "#DBCS LEAD BYTE" else 20
		else:
			table[code] = column[1].hex_to_int()

var first_table : Array[int]
