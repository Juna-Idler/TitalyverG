...
static func decode_ncr(text : String) -> String:
	var named_character_reference_regex := RegEx.create_from_string("&\\w+;")
	var d_numeric_character_reference_regex := RegEx.create_from_string("&#(\\d+);")
	var h_numeric_character_reference_regex := RegEx.create_from_string("&#[xX]([0-9a-fA-F]+);")	
	
	var ncr := named_character_reference_regex.search(text)
	while ncr:
		var n := ncr.get_string()
		var r : String = TABLE[n]["characters"]
		text = text.replace(n,r)
		ncr = named_character_reference_regex.search(text,ncr.get_start() + 1)
	
	ncr = d_numeric_character_reference_regex.search(text)
	while ncr:
		var n := int(ncr.get_string(1))
		text = text.replace(ncr.get_string(),char(n))
		ncr = d_numeric_character_reference_regex.search(text,ncr.get_start() + 1)
		
	ncr = h_numeric_character_reference_regex.search(text)
	while ncr:
		var n := ("0x" + ncr.get_string(1)).hex_to_int()
		text = text.replace(ncr.get_string(),char(n))
		ncr = h_numeric_character_reference_regex.search(text,ncr.get_start() + 1)
	return text
