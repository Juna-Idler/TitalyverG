...
class_name BuiltinBlurImageProcessor

var canvas : Control
var texture_rect : TextureRect
var color_rect : ColorRect

const SIPCanvas := preload("res://image/builtin_processor/blur_processor_canvas.tscn")
const SIPSettings := preload("res://image/builtin_processor/blur_processor_settings.tscn")
...
func _get_name() -> String:
	return ImageManager.BUILTIN_BLUR_IMAGE_PROCESSOR
...
func _get_settings(config : ConfigFile) -> Control:
	...
	var shader_param : SpinBox = settings.get_node("%ButtonShaderParam")
	shader_param.value_changed.connect(func(v):
			(texture_rect.material as ShaderMaterial).set_shader_parameter("strength",v)
			)
	return settings
...
func set_texture_opacity(rate : float):
	(texture_rect.material as ShaderMaterial).set_shader_parameter("alpha",rate / 100)
